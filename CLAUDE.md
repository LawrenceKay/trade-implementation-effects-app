...
|---------|----------|-----------------|--------|
| WB DTA 2.0 — Bilateral Information | World Bank | Country-pair-year panel keyed by WBID; sole source of FTA network topology, depth, and eigenvector centrality (see CLAUDE.md Section 5, "Base network source mismatch") | `datatopics.worldbank.org/dta` → Saved to `data/raw/DTA 2.0 - Vertical Content (v2).xlsx` |
...
- **Rewrite the pseudocode/docstrings in `centrality_pipeline.py` and `network_example.py` in Lawrence's own words, and link this to the AI-disclosure convention above.** Both modules' step-by-step comments and docstrings currently read in a generic, AI-generated style; rewriting them in Lawrence's own language is itself the kind of substantive modification the "Adopt AI-disclosure convention" item above says should carry an `SPDX-AI-Disclosure:` header going forward — do the two together rather than as separate passes. For `network_example.py` — the larger, user-facing Streamlit app rather than a single-purpose pipeline script — take the opportunity to expand the explanatory comments/docstrings rather than a like-for-like rewrite.

#### Evaluation

- **Evaluate the app, and evaluate evaluation tools themselves, perhaps using Langsmith, Arize, Maze, or Posthog — including running the codebase through an evaluation agent (e.g. `/code-review`), not yet done since the DESTA→WB DTA 2.0 migration and the other Section 5 fixes above.** Covers three dimensions: code-level review via `/code-review`; LLM/agent-eval platforms (Langsmith, Arize) for grading the Trade Economist agent's output faithfulness once built (item 14); and usability-testing platforms (Maze, Posthog) for the app's UI. Needs a decision on which tool(s) are worth the setup cost for an app this size, not just which are best in the abstract.
...
- **Consider the colouring of recommended countries.** Recommended-country names in the analysis paragraph and score-card notes currently use `COLOR_RED` as a highlight against the teal theme. Worth reviewing whether red is the right choice long-term (e.g. against the validated `#FF8D70` alternative surfaced during the teal-contrast colour-formula exercise on 2026-07-15) versus keeping red.
- **Add the ability to add the selected country to the bottom boxes.** The "Recommended trade partners" panels at the bottom of the page (depth/centrality/complexity) currently list recommended non-partner countries only, with no direct way to add one to the "Current trade agreement partners" group-simulation box from within the panel itself — the user has to switch back up to the selector and type the name. Consider adding a click-to-add action from each recommended-country row into the group-simulation box.
- **Clarify "Non-partners whose centrality would most lift your own."** This subtitle under the centrality recommendation panel doesn't mention that clicking/selecting a listed country adds it to the group-simulation box — reword so the connection to country selection is explicit.
- **Make "simulated additionality · +0.00 vs current · centrality multiplier ×1.00" clearer.** This score-card note is dense and jargon-heavy as currently worded; needs a plainer-language rework so a non-technical user can tell what it means at a glance.
- **Drop suggested countries from the bottom recommendation panels once selected as extra partners.** Once a user adds a recommended country to the group-simulation box, it should no longer appear in the "Recommended trade partners" candidate lists below — currently it stays listed even after being added.
- **De-capitalise the analysis section.** Review the country-analysis text/headings for over-capitalisation and switch to normal sentence case where appropriate.
- **Make the analysis section easier to engage with.** Currently a dense block of prose; consider ways to make it more scannable/interactive (e.g. breaking it up, highlighting key figures, or other presentation changes).
- **Remove the "Recommended trade partners" heading.** The section label above the three recommendation panels is redundant with the panels' own titles; remove it.
