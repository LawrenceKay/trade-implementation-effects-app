...
- **Depth weighting**: centrality pipeline uses binary edge weights (agreement present = 1). Fan et al. use Hofmann et al. (2017) depth scores (0–48 scale) from WB DTA 2.0. `data/raw/DTA 1.0 - Horizontal Content (v2).xlsx` is only DTA **1.0**, an older, smaller-coverage vintage than the DTA 2.0 (280+ agreements) named as the target source in Section 4 — needs re-downloading from `datatopics.worldbank.org/dta`, then joining to DESTA by treaty ID.
- **Base network source mismatch**: Fan et al.'s network (who is connected to whom, not just edge depth) is built directly from World Bank DTA data. Ours is built from **DESTA** (`desta_dyads.csv`), with WB DTA layered on only as an optional depth overlay. DESTA and WB DTA catalogue overlapping but non-identical sets of agreements, so our network topology can differ from theirs before depth-weighting is even considered.
- **"Non-natural" split is a coarser proxy**: Fan et al. classify a partner as "natural" using actual bilateral geographic distance vs. the sample average. `centrality_pipeline.py`'s `split_natural_nonnatural()` instead flags same-World-Bank-region as "natural" — a reasonable but not equivalent test (e.g. two same-region countries far apart would count as "natural" for us but might not for them).
- **"Developed"/"developing" sub-network split not implemented (H3)**: Fan et al.'s strongest result (β=3.514***) comes from splitting centrality into `ED_Centrality`/`ING_Centrality` by WB income classification (excluding oil-rich states). `centrality_pipeline.py` only outputs `overall_centrality`, `nn_centrality`, `n_centrality` — there is no developed/developing split anywhere in the pipeline or app.
- **ECI vintage and method**: ECI data is 2012 (most recent year in the public CID GitHub archive), a single cross-section vs. Fan et al.'s 2000–2019 panel. It's also a Harvard-precomputed value (`new_observatory_cy.csv`) rather than something we derive ourselves from product-level export data via the method of reflections (Hausmann & Hidalgo, 2011) — so it can't respond to the app's "group simulation" feature the way Fan et al.'s mechanism implies it should.
- **Missing control-variable data stack**: none of Fan et al.'s regression controls or robustness-check data are in the repo — no WDI population/human-capital/natural-resource-rent/financial-development/openness series, no BLS production-capability crosswalk, no IIA Navigator bilateral investment treaty counts, no CEPII gravity data (distance/language/colony/border). `wb_cache.csv` holds WB *governance* indicators for the app's separate institutional-quality scoring (`score.py`), which is not a variable in Fan et al.'s model at all. Needed if the regression below is ever implemented.
...
- **Regression not implemented**: Fan et al.'s core result (ECI ~ Centrality + controls) has not been replicated. The centrality scores and ECI data are both present and could be used for a regression with WDI controls, once the control-variable gap above is closed.
