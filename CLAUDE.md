...
- **Consider colouring the bottom score cards (Group centrality / Agreement depth / Complexity additionality) as they change.** When the user adds simulated partners in the group-selection box, these three scores update but currently render in a single static colour regardless of direction. Consider colour-coding increases/decreases (e.g. green for an improvement, red for a decline) so the effect of adding a partner is visible at a glance, not just from the number itself.
